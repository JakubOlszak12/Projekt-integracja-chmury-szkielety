...
const EditLaureate = () => {
    ...
    useEffect(() => {
        ...
        async function fetchLaureateInfo(){
            const id = /[^/]*$/.exec(window.location.href)[0];
            const url = "http://localhost:8000/api/getLaureateInfo/"+id
            const token = localStorage.getItem("token");
            await axios.get(url, {
                headers: {
                    'Authorization': `Bearer ${token}`
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const id = /[^/]*$/.exec(window.location.href)[0];
            const url = "http://localhost:8000/api/updateLaureate/"+id
        ...
        <Wrapper className='full-page'>
           <form className='form'
                        ...
                        <label htmlFor="gender" className='form-label'>
                         ...
                         <input name="birth_date" type="date" onChange={handleChange} value={data.birth_date}/>
